...
function _spelled_out_en!(
    io::IOBuffer, number_norm::Integer; british::Bool=false, dict::Symbol=:modern
)
    ...
    if number_norm > limit - 1
        error(
            """SpelledOut.jl does not support numbers larger than $(limit_str * " - 1").  Sorry about that!""",
        )
    end
...
function decimal_convert_en(
    number::AbstractString; british::Bool=false, dict::Symbol=:modern
)
...
function spelled_out_en(number::Complex; british::Bool=false, dict::Symbol=:modern)
    return spelled_out_en(real(number); british=british, dict=dict) *
           " and " *
           spelled_out_en(imag(number); british=british, dict=dict) *
           " imaginaries"
end
...
function spelled_out_en(number::Rational; british::Bool=false, dict::Symbol=:modern)
    ...
    word =
        spelled_out_en(_num; british=british, dict=dict) *
        " " *
        spell_ordinal_en(_den; british=british, dict=dict)

    # account for pluralisation
    return isone(_num) ? word : word * "s"
end

function spelled_out_en(
    number::AbstractIrrational; british::Bool=false, dict::Symbol=:modern
)
    throw(
        error(
            "Please round the input number, as we support floating point printing but cannot support irrational printing.",
        ),
    )
end
