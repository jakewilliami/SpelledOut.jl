...
function es_spell_decimal(number, partitive=true)
    ...
    if partitive
        if decimal_length > 6
            throw(
                error(
                    """partitive pronunciation of spanish decimals is only supported to up to six decimals, the decimal part is $decimal .
        ...
        res = (z -> (z -> join(z, " "))(reverse(strip.(es_spell_1e3(z, true))))).(digits(
            decint
        ))
        res = "punto " * res
    end
...
function es_spell_large(_number)
    ...
    res = (z -> join(z, " "))(reverse(strip.(es_spell_large_map.(list, 1:length(list)))))
