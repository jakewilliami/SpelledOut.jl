...
function split_numbers_10³(num::Integer)
    return digits(num; base=10^3)
end

function split_numbers_10⁶(num::Integer)
    return digits(num; base=10^6)
end
...
function pt_spell_decimal(number, partitive=true; portugal::Bool=false)
    ...
    if partitive
        if decimal_length > 6
            throw(
                error(
                    """partitive pronunciation of Portuguese decimals is only supported to up to six decimals, the decimal part is $decimal .
        ...
        res = (z -> (z -> join(z, " "))(reverse(strip.(pt_spell_1e3(
            z; short_one=true, portugal=portugal
        ))))).(digits(decint))
        res = "ponto " * res
    end
end
...
function pt_spell_large_map(number, i; portugal::Bool=false)
    ...
    elseif portugal & isone(number)
        ...
        return pt_spell_1e6(number; short_one=false, portugal=portugal) *
               " " *
               pt_PT_multiplos_1e6_plural[i - 1]
    end
end
...
function pt_spell_large(_number; portugal::Bool=false)
    ...
    if portugal
        res = (z -> join(z, " "))(reverse(strip.(pt_spell_large_map.(
            list, 1:length(list), portugal=portugal
        ))))
        (_number < 0) && (res = "menos " * res)
    else
        res =
            (z -> join(z, " "))(reverse(strip.(pt_spell_large_map.(list, 1:length(list)))))
        (_number < 0) && (res = "menos " * res)
    end
    return res
end
