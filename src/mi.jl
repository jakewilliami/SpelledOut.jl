...
function _fn_bounds_err_msg(n::I, bounds::String, fname::Symbol) where {I<:Integer}
    return "The internal function \"$fname\" should only be used for numbers $bounds; number provided: $n"
end
...
function _teens_convert_mi(n::I) where {I<:Integer}
    @assert(
        10 < n < 20,
        _fn_bounds_err_msg(n, "10 < n < 20", Base.StackTraces.stacktrace()[1].func)
    )
...
function _tens_convert_mi(n::I) where {I<:Integer}
    @assert(
        20 ≤ n < 100,
        _fn_bounds_err_msg(n, "20 ≤ n < 100", Base.StackTraces.stacktrace()[1].func)
    )
    d, r = divrem(n, 10)
...
function _hundreds_convert_mi(n::I) where {I<:Integer}
    @assert(
        100 ≤ n < 1000,
        _fn_bounds_err_msg(n, "100 ≤ n < 1000", Base.StackTraces.stacktrace()[1].func)
    )
    d, r = divrem(n, 100)
